...
<form id="form-category-grids" method="POST" action="{{route('grids-category.store', $id)}}" onsubmit="return false">
    ...
    <p class="button-height inline-small-label">
        ...
        <span class="button-group">
                    <label for="type-1" onclick="typeGrideCategory('unit')" class="button blue-gradient green-active">
                        <input type="radio" name="type" id="type-1" value="unit">Unidade
                    </label>
                    <label for="type-2" onclick="typeGrideCategory('kit')" class="button blue-gradient  green-active">
                        <input type="radio" name="type" id="type-2" value="kit">Embalagem
                    </label>
                    <label for="type-3" onclick="typeGrideCategory('kit')" class="button blue-gradient green-active">
    ...
    <div id="grid-unit" style="display: none">
        <p class="button-height inline-small-label">
            ...
            <span class="puls_grid input margin-right">
                        ...
                        <button onclick="removeGridCategory(this,'.puls_grid')" class="remove-unit button red-gradient icon-minus" title="Remover"></button>
                    </span>
            <span id="plus-unit"></span>

            <button onclick="plusUndCategory()" class="button blue-gradient icon-plus" title="Adicionar"></button>
    ...
    <div id="grid-kit" style="display: none">
        <p class="puls_grid button-height inline-small-label">
                    <span class="input">
                        ...
                        <button onclick="removeGridCategory(this,'.puls_grid')" class="remove button red-gradient icon-minus" title="Remover"></button>
                    </span>
        </p>
        <div id="plus-kit"></div>
        <div class="large-margin-left margin-top">
            <button id="total-grids" class="button blue-gradient">0</button>
            <button onclick="plusKitCategory()" class="button blue-gradient icon-plus" title="Adicionar"></button>
            @can('category-grids-create')
                <button id="btn-modal" onclick="formGridCategory('{{$id}}','create', 'category-grids', '{{route('category-grids-load', $id)}}', 'Aguarde', 'Salvar')" class="button blue-gradient">
                    <span class="icon-tick"></span> Salvar
                </button>
        ...
        <div id="grid_{{$val->id}}">
            <h3 class="thin underline margin-top">
                ...
                <span class="button-group compact">
                        ...
                        <button id="btn-delete" onclick="deleteGridCategory('{{ $val->id }}', '{{ $val->name }}', '{{route('grids-category.destroy', ['id' => $id, 'grid' => $val->id])}}', '{{ csrf_token() }}');" class="button icon-trash with-tooltip red-gradient" title="Excluir Grade"></button>
                    @endcan
                        </span>
            </h3>
