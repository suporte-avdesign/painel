	...
	<ul id="gallery-{{$type}}" class="gallery">
		@foreach($data as $file)
			@if($type == $file->type)
				...
				<script>$("#order-{{$file->id}}").menuTooltip("Carregando...",{classes:["with-mid-padding"],ajax:"imagens/banner/order/{{$file->id}}",onShow:function(e){e.parent().removeClass("show-on-parent-hover")},onRemove:function(e){e.parent().addClass("show-on-parent-hover")}});</script>
			@endif
		@endforeach
	</ul>
	<script src="{!! url('assets/backend/scripts/banners.js?'.time()) !!}"></script>
@endcan