        ...
        <dl class="accordion same-height">
            @forelse($data as $value)
                ...
                <div class="button-group absolute-right compact margin">
                    <button id="btn-reactivate-{{$value->id}}" onclick="formPaymentsReactivate('{{$value->id}}', '{{route('form-payments.reactivate', $value->id)}}')" class="button icon-tick with-tooltip blue-gradient" title="Ativar">Reativar</button>
                </div>
                </dt>
                <dd>
