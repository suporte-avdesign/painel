<div class="block-title silver-gradient">
	...
	<div class="button-group absolute-right compact">
		...
		@can('config-status-payments-create')
		    <button 
    			onclick="abreModal('{{$title_create}}', '{{route('status-pagamentos.create')}}', 'name', 2, true, 400, 350)"
			...
			@foreach($data as $value)
				<dt id="status-payments-{{$value->id}}"><strong>{{$value->order}}</strong> - {{$value->label}}
					<div class="button-group absolute-right compact margin">
						...
						<button onclick="abreModal('{{$title_edit}}', '{{route('status-pagamentos.edit', $value->id)}}', 'status-payments', 2, true, 400, 350)" class="button icon-pencil">Editar</button>
...
<script src="{{url('assets/backend/scripts/settings/status-payments.js')}}?{{time()}}"></script>
<script>
