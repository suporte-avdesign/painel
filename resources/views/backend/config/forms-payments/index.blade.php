<div class="block-title silver-gradient">
	...
	<div class="button-group absolute-right compact">
		@can('config-form-payment-delete')
			<button id="btn-excluded" onclick="formPaymentsExcluded('{{route('form-payments.excluded')}}')" class="button icon-trash margin-left red-gradient glossy ">Excluidos</button>
		@endcan
		@can('config-form-payment-create')
		    <button 
    			onclick="abreModal('{{$title_create}}', '{{route('forma-pagamentos.create')}}', 'name', 2, true, 400, 250)"
			...
			@foreach($data as $value)
				<dt id="payment-{{$value->id}}"><strong>{{$value->order}}</strong> - {{$value->label}}
					<div class="button-group absolute-right compact margin">
						<a href="javascript:deleteFormPayments('payment-{{$value->id}}', '{{route('forma-pagamentos.destroy', $value->id)}}', '{{csrf_token()}}')" class="button icon-trash with-tooltip confirm red-gradient" title="Excluir"></a>
						<button onclick="abreModal('{{$title_edit}}', '{{route('forma-pagamentos.edit', $value->id)}}', 'payments', 2, true, 400, 250)" class="button icon-pencil">Editar</button>
...
<script src="{{url('assets/backend/scripts/settings/forms-payments.js')}}?{{time()}}"></script>
<script>
