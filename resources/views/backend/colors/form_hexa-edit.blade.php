    ...
    <p class="button-height inline-small-label">
        ...
        <span class="button-group">
            <label for="active_img_{{$idpro}}_1" class="button green-active">
                <input type="radio" name="img[active]" id="active_img_{{$idpro}}_1" value="{{constLang('active_true')}}" @if($data->active == constLang('active_true')) checked @endif>
                Ativo
            </label>
            <label for="active_img_{{$idpro}}_2" class="button red-active" >
                <input type="radio" name="img[active]" id="active_img_{{$idpro}}_2" value="{{constLang('active_false')}}" @if($data->active == constLang('active_false')) checked @endif>
                Inativo
            </label>
        </span>
    </p>
