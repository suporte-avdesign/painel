...
<ul id="gallery-{{$id}}" class="gallery">
	@foreach($data as $file)
		@if($upload['type'] == $file->type)
			<li class="square" id="img-{{$file->id}}">
				...
				<div class="controls">
					<span id="btns-{{$file->id}}" class="button-group compact children-tooltip">
						...
						@endif
						<button id="order-{{$file->id}}" class="button" title="{{$upload['btn']['edit']}}">{{$upload['btn']['order']}} ({{$file->order}})</button>
			...
			<script>$("#order-{{$file->id}}").menuTooltip("Carregando...",{classes:["with-mid-padding"],ajax:"imagens/{{$file->id}}/slider/order",onShow:function(e){e.parent().removeClass("show-on-parent-hover")},onRemove:function(e){e.parent().addClass("show-on-parent-hover")}});</script>
		@endif
	@endforeach
</ul>
<script src="{!! url('assets/backend/scripts/sliders.js?'.time()) !!}"></script>