<div class="block large-margin-bottom">
    ...
    <div class="block-title">
        <h3><span class="icon-credit-card"></span> {{$config['title_index']}}</h3>
        @can('contents-site-create')
            <div class="button-group absolute-right compact">
                <button onclick="abreModal('{{$config['title_create']}}', '{{route('form-payment.create')}}', 'contents', 2, true, 600, 700)" title="{{$config['title_create']}}" class="button icon-plus-round with-tooltip anthracite-gradient">Adicionar</button>
            </div>
        @endcan
    </div>
        ...
        <li id="content-{{$value->id}}">
            ...
            <div class="button-group compact">
                @can('contents-site-update')
                    ...
                    <button id="status-{{$value->id}}" onclick="statusContent('{{$value->id}}','{{route('form-payment.status', $value->id)}}','{{csrf_token()}}')" title="Alterar Status" class="button icon-tick with-tooltip @if($value->status == 'Inativo')red @endif">{{$value->status}}</button>
                @endcan
