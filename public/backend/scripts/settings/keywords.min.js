...
;(function($)
{   
    $.fn.loadTableKeywords = function()
    {
        var detals = Handlebars.compile($("#"+avdTable.id+"-template").html()),
        ...
        $('#'+avdTable.id).on('click', avdTable.openDetails, function(event) {
          if (event.target !== this){
            ...
            if (row.child.isShown()) {
                ...
                // Abrir esta row
                row.child(detals(row.data())).show();
                tr.addClass('shown');
        ...
        */
        deleteKeyword = function(url, name)
        {
