<div class="block-title silver-gradient">
    ...
    @can('config-keyword-ceate')
        <span class="button-group absolute-right with-tooltip">
            <button 
                onclick="abreModal('{{$title_create}}', '{{route('images.create')}}', 'config-images', 2, true, 400, 250)" 
...
<script src="{{url('assets/backend/scripts/settings/images-products.js')}}?{{time()}}"></script>
<script>
    var avdTable = {!! json_encode([
        "id" => 'config-images',
        "url" => route('config.images.data'),
