        ...
        <li id="content-{{$value->id}}">
            ...
            <div class="button-group compact">
                @can('contents-site-update')
                    ...
                    <button id="status-{{$value->id}}" onclick="statusContent('{{$value->id}}','{{route('terms-conditions.status', $value->id)}}','{{csrf_token()}}')" title="Alterar Status" class="button icon-tick with-tooltip @if($value->active == constLang('active_false'))red @endif">{{$value->active}}</button>
                @endcan
