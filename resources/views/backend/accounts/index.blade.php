<div class="block-title">
    ...
    <div class="button-group absolute-right">
        @can('account-delete')
            <a href="clientes/excluidos" class="button icon-users margin-left red-gradient glossy ">Excluidos</a>
        @endcan

        @can('account-create')
            <a href="javascript:void(0)" onclick="abreModal('Adicionar Cliente', '{{route('clientes.create')}}', 'accounts', 2, 'true', 500, 800);" class="button margin-right with-tooltip" data-tooltip-options='{"classes":["anthracite-gradient"],"position":"bottom"}' title="Adicionar Usuários">Adicionar       <span class="button-icon right-side"><span class="icon-plus-round"></span></span></a>
        @endcan
...
<script src="{!! url('assets/backend/scripts/accounts.js?'.time()) !!}"></script>
<script>
                    ...
                    <ul class="list">
                        <li>
                            <div class="button-group absolute-right compact">
                                @can('account-delete')
                                    <button id="btn-excluir" onclick="deleteAccount('@{{{id}}}','clientes/@{{{id}}}', '@{{{first_name}}}');" class="button icon-trash with-tooltip red-gradient" title="Excluir Cliente">Excluir
                                    </button>
                ...
                <div class="with-padding white-gradient ">
                    <h4 class="blue underline">
                        ...
                        <span class="button-group float-right compact">
                            @can('account-update')
                                <button id="btn-editar" onclick="abreModal('Alterar Cliente: @{{{first_name}}}', 'clientes/@{{{id}}}/edit', 'dados-clientes', 2, 'true', 400, 500);" class="button blue-gradient icon-pencil with-tooltip" title="Alterar Cliente">
                                    Editar
                                </button>
                            @endcan
                        </span>
