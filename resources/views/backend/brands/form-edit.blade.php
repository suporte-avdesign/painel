				...
				<p class="button-height inline-label">
					...
					<span class="button-group">
						<label for="status_logo-1" class="button blue-active">
							<input type="radio" name="status_logo" id="status_logo-1" value="Ativo" @if($data->status_logo == 'Ativo') checked @endif>
							Ativo
						</label>
						<label for="status_logo-0" class="button red-active">
							<input type="radio" name="status_logo" id="status_logo-0" value="Inativo" @if($data->status_logo == 'Inativo') checked @endif>
							Inativo
						</label>
					</span>
				</p>
					...
					<p class="button-height inline-label">
						...
						<select name="state" class="select">
							...
							@foreach($options as $key => $val)
								<option value="{{$key}}" 
								@if($data->state == $val) selected @endif> {{$val}} </option>
							@endforeach
						</select>
					</p>
					<p class="button-height inline-label">
